...
`ten_ dich_vu_di_kem` varchar(45),
`gia`double,
`don_vi`varchar(10),
...
insert into dich_vu_di_kem(ma_dich_vu_di_kem,`ten_ dich_vu_di_kem`,gia,don_vi,trang_thai)
values
...
or round(datediff(curdate(),ngay_sinh)/365,0) >=18 )
;

alter table khach_hang add column `so_lan_dat_phong`int not null
;

update khach_hang set so_lan_dat_phong = + 1
where hop_dong.ma_khach_hang = khach_hang.ma_khach_hang
;
select ma_khach_hang, ho_ten, so_lan_dat_phong
from khach_hang
where so_lan_dat_phong != null 
and ma_loai_khach =1
;